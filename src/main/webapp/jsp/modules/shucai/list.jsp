...
<body>
    <div class="modal fade" id="plusOrReduceShucaiKucunNumberModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <%-- 模态框标题--%>
                <div class="modal-header">
                    <h5 class="modal-title" id="plusOrReduceShucaiKucunNumberTitle">增加或减少库存</h5>
                ...
                <div class="modal-body">
                    数量:<input type="number" id="plusOrReduceShucaiKucunNumber" style="width: 300px;" class="form-control form-control-sm"
                    ...
                    <input type="hidden" id="shucaiId">
                    <input type="hidden" id="hideflag">
                    <input type="hidden" id="thisShucaiKucunNumber">
                </div>
                <%-- 模态框按钮 --%>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                    <button type="button" onclick="submitPlusOrReduceShucaiKucunNumber()" class="btn btn-primary">提交</button>
                </div>
            </div>
        </div>
    </div>
...
<!--/Pre Loader -->
                ...
                <div class="row breadcrumb-bar">
                    ...
                    <div class="col-md-6">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">蔬菜列表</li>
                        </ol>
                    </div>
                </div>
            </div>
                            ...
                            <div class="table-responsive mb-3">
                                <div class="col-sm-12">
                                    ...
                                    <div class="layui-inline" style="margin-bottom: 10px;">
                                        蔬菜编号
                                        <div class="layui-input-inline">
                                            <input type="text" id="shucaiUuidNumberSearch" style="width: 140px;" class="form-control form-control-sm"
                                                   placeholder="蔬菜编号" aria-controls="tableId">
                                        </div>
                                    </div>
    ...
    <!-- 本表的查询条件 -->
        ...
        function setDataRow(item, number) {
            ...
            //蔬菜编号
            var shucaiUuidNumberCell = document.createElement('td');
            shucaiUuidNumberCell.innerHTML = item.shucaiUuidNumber;
            row.appendChild(shucaiUuidNumberCell);
            ...
            //蔬菜库存
            var shucaiKucunNumberCell = document.createElement('td');
            shucaiKucunNumberCell.innerHTML = item.shucaiKucunNumber;
            row.appendChild(shucaiKucunNumberCell);


            //购买获得积分
            var shucaiPriceCell = document.createElement('td');
            shucaiPriceCell.innerHTML = item.shucaiPrice;
            row.appendChild(shucaiPriceCell);


            //蔬菜原价
            var shucaiOldMoneyCell = document.createElement('td');
            shucaiOldMoneyCell.innerHTML = item.shucaiOldMoney;
            row.appendChild(shucaiOldMoneyCell);


            //现价
            var shucaiNewMoneyCell = document.createElement('td');
            shucaiNewMoneyCell.innerHTML = item.shucaiNewMoney;
            row.appendChild(shucaiNewMoneyCell);


            //蔬菜热度
            var shucaiClicknumCell = document.createElement('td');
            shucaiClicknumCell.innerHTML = item.shucaiClicknum;
            row.appendChild(shucaiClicknumCell);
            ...
            var plusAttr = "plusShucaiKucunNumber(" + item.id+','+item.shucaiKucunNumber+")";
            ...
            var reduceAttr = "reduceShucaiKucunNumber(" + item.id+','+item.shucaiKucunNumber+')';
        ...
        function remove(id) {
            ...
            if (mymessage == true) {
                ...
                httpJson("shucai/delete", "POST", paramArray, (res) => {
                    if(res.code == 0){
                        getDataList();
                        alert('删除成功');
                    }
        ...
        //添加
        function add() {
            window.sessionStorage.setItem("addshucai", "addshucai");
            window.location.href = "add-or-update.jsp"
        }
        ...
        function shangxia(id,shangxiaTypes) {
            ...
            httpJson("shucai/update", "POST", data, (res) => {
                if(res.code == 0){
                    alert(msg);
                    getDataList()
                }else{
    ...
    //查询当前表搜索条件所有列表
        ...
        //增加库存
        function plusShucaiKucunNumber(thisId,shucaiKucunNumber) {
            if(thisId == null || thisId=="" || thisId=="null"){
                alert("未知错误,请联系管理员处理");
                return;
            }
            $("#plusOrReduceShucaiKucunNumberTitle").val("增加库存");
            $("#thisShucaiKucunNumber").val(shucaiKucunNumber);
            $("#hideflag").val("+");
            $("#shucaiId").val(thisId);
            id=thisId;
            $('#plusOrReduceShucaiKucunNumberModal').modal('show');
        }

        //减少库存
        function reduceShucaiKucunNumber(thisId,shucaiKucunNumber) {
            if(thisId == null || thisId=="" || thisId=="null"){
                alert("未知错误,请联系管理员处理");
                return;
            }
            $("#plusOrReduceShucaiKucunNumberTitle").val("减少库存");
            $("#thisShucaiKucunNumber").val(shucaiKucunNumber);
            $("#hideflag").val("-");
            $("#shucaiId").val(thisId);
            id=thisId;
        ...
        $('#plusOrReduceShucaiKucunNumberModal').on('show.bs.modal', function () {
            //清理表格内容,防止重复回显
            $("#plusOrReduceShucaiKucunNumber").val(0);
            if(id ==null){
                alert("获取当前列id错误,请联系管理员处理");
                $('#plusOrReduceShucaiKucunNumberModal').modal('hide');
                $("#plusOrReduceShucaiKucunNumberTitle").val("");
                $("#thisShucaiKucunNumber").val("");
                $("#hideflag").val("");
                $("#shucaiId").val("");
                getDataList();
                return;
        ...
        //模态框关闭的时候会执行这个里面的东西
        $('#plusOrReduceShucaiKucunNumberModal').on('hide.bs.modal', function () {
            id=null;
            $("#plusOrReduceShucaiKucunNumber").val(0);
            $("#plusOrReduceShucaiKucunNumberTitle").val("");
            $("#thisShucaiKucunNumber").val("");
        ...
        function submitPlusOrReduceShucaiKucunNumber() {
            var id = $("#shucaiId").val();//当前数据id
            var hideflag = $("#hideflag").val();//是加还是减
            var thisShucaiKucunNumber = $("#thisShucaiKucunNumber").val();//原先的数量
            var plusOrReduceShucaiKucunNumber = $("#plusOrReduceShucaiKucunNumber").val();//增加或者减少的数量
            var sumNumber=0;
            var msg;
            if(hideflag == "+"){
                sumNumber =parseInt(thisShucaiKucunNumber) + parseInt(plusOrReduceShucaiKucunNumber);
                msg = "增加库存成功";
            }else if(hideflag == "-"){
                sumNumber =parseInt(thisShucaiKucunNumber) - parseInt(plusOrReduceShucaiKucunNumber);
                msg = "减少库存成功";
            ...
            let data = {"id":id,"shucaiKucunNumber":sumNumber};
            httpJson("shucai/update", "POST", data, (res) => {
                if(res.code == 0){
                    alert(msg);
                    getDataList();
