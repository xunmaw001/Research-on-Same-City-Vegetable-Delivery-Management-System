    ...
    <body>
                ...
                <table class="layui-table" lay-skin="nob" style="border: 3px dotted #EEEEEE;margin: 20px 0;">
                    ...
                    <tbody>
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            ...
                            <td  style="width: 150px;">{{item.shucaiNewMoney}} RMB</td>
                            <td style="width: 140px;">
                                {{item.buyNumber}}
                            </td>
                            <td style="width: 100px;">{{(item.shucaiNewMoney*item.buyNumber).toFixed(2)}} RMB</td>
                        </tr>
                    </tbody>
                </table>

                <div class="btn-container">
                    <div v-if="this.shucaiOrderPaymentTypes == 1" style="margin-right: 150px">
		...
		<script>
				...
				computed: {
					totalMoney: function() {
						...
						for (var item of this.dataList) {
							total += item.shucaiNewMoney * item.buyNumber
						}
						return total;
					}
				},
				methods: {
					...
					,payClick() {
						...
						let data={
                            ...
                            shucais:localStorage.getItem('shucais'),
                            yonghuId: localStorage.getItem('userid'),
                            shucaiOrderPaymentTypes: vue.shucaiOrderPaymentTypes,
						}
                        // 获取商品详情信息
                        layui.http.request(`shucaiOrder/order`, 'POST', data, (res) => {
                            // 订单编号
                            var orderId = genTradeNo();
							if(res.code == 0){
                                alert("下单成功，点击确定后跳转 我的订单页面");
                                window.location.href='../shucaiOrder/list.jsp';
							}
                       		 window.location.href='../shucaiOrder/list.jsp';
					    });
					}
			...
			layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload'], function() {
                ...
                // 获取蔬菜购买的清单列表
				var shucais = localStorage.getItem('shucais');
				// 转换成json类型，localstorage保存的是string数据
				vue.dataList = JSON.parse(shucais);
