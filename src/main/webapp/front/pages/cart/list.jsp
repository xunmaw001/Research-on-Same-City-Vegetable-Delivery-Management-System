        ...
        <table class="layui-table" lay-skin="nob">
            ...
            <tr v-for="(item,index) in dataList" v-bind:key="index">
                <td style="display: flex;text-align: left;">
                    <img :src="item.shucaiPhoto" style="width: 50px;height: 50px;object-fit: cover;">
                    <div style="margin-left: 10px;margin-top: 10px;">
                        {{item.shucaiName}}
                    </div>
                </td>
                <td style="width: 200px;">{{item.shucaiNewMoney}} RMB</td>
                ...
                <td style="width: 200px;">{{(item.shucaiNewMoney*item.buyNumber).toFixed(2)}} RMB</td>
    ...
    var vue = new Vue({
        ...
        methods: {
			...
			,addCartNum(index) {
				...
				layui.http.request(`shucai/info/${item.shucaiId}`, 'get', {}, function(res) {
					// 库存限制
					if (res.data.shucaiKucunNumber && res.data.shucaiKucunNumber > 0 && res.data.shucaiKucunNumber  <= item.buyNumber) {
						layui.layer.msg(`蔬菜库存不足`, {
							time: 2000,
							icon: 5
						});
						return
			...
			// 购买
			buyClick() {
				localStorage.setItem('shucais', JSON.stringify(this.dataList));
				window.location.href = '../shucaiOrder/confirm.jsp'
			}
        }
    });
