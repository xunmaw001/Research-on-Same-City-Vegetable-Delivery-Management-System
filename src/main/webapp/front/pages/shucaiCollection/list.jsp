            ...
            <div class="ng-inmod-ini">
                    <div class="ng-fault ng-container">
                        <div class="ng-inmod-the ng-container">
                            ...
                            <span class="ng-inmod-the-cn" aos="fade-down">蔬菜收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="shucaiName" id="shucaiName" placeholder="蔬菜名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="shucaiUuidNumber" id="shucaiUuidNumber" placeholder="蔬菜编号" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="shucaiTypes" id="shucaiTypes" lay-filter="shucaiTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in shucaiTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <i @click="deleteShucaiCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: -10px;font-size:20px">&#xe640;</i>
                                    <div class="ng-prslist-img" @click="jumpCheck('../shucai/detail.jsp?id='+item.shucaiId , item.shucaiDelete == null?'':item.shucaiDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)" >
                                        <a href="#" class="ng-prsliik">
                                            <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                                <img class="swiper-item" :src="item.shucaiPhoto?item.shucaiPhoto.split(',')[0]:''" style="height: 350px;width:100%;"/>
                                            </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jumpCheck('../shucai/detail.jsp?id='+item.shucaiId , item.shucaiDelete == null?'':item.shucaiDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)" ><span>{{item.shucaiName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.shucaiNewMoney"><span style="color: red;">{{item.shucaiNewMoney}} RMB</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            shucaiCollectionTypesList : [],
        // 级联表字典表数据
            shucaiTypesList : [],
            shangxiaTypesList : [],
            shucaiCollectionTypes: 0,//类型
            limit: 8,
        ...
        methods: {
            ...
            ,deleteShucaiCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('shucaiCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        // 查询当前字典表相关
            // 类型的查询和初始化
        shucaiCollectionTypesSelect();
    // 当前表的字典表
        // 类型的查询
        function shucaiCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=shucai_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.shucaiCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function shucaiTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=shucai_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.shucaiTypesList = res.data.list;
                }
            });
        }
        ...
        function pageList() {
            ...
            if (jquery('#shucaiName').val() != null && jquery('#shucaiName').val() != "" && jquery('#shucaiName').val() != "null" ) {
                param['shucaiName'] = jquery('#shucaiName').val();
            }
                     
            if (jquery('#shucaiUuidNumber').val() != null && jquery('#shucaiUuidNumber').val() != "" && jquery('#shucaiUuidNumber').val() != "null" ) {
                param['shucaiUuidNumber'] = jquery('#shucaiUuidNumber').val();
            }
                                         
            var shucaiTypes = $("#shucaiTypes").find("option:selected").val();
            if (shucaiTypes != null && shucaiTypes != "" && shucaiTypes != "null") {
                            param['shucaiTypes'] = shucaiTypes;
                        }
                                                                                                                                                                                    
            // 当前表
                                                
            //收藏只能查类型为1的
            param['shucaiCollectionTypes'] = 1;

            // 获取列表数据
            http.request('shucaiCollection/page', 'get', param, function(res) {
